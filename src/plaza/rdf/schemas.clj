...
(defn- build-pattern-for-model
  ...
  (let [opt-props-to-add (vec (clojure.set/difference (set (keys properties)) (set props)))
        ...
        mandatory-pattern (reduce
                           (fn [ac it]
                             (when (nil? (it properties))
...
(deftype RDFSModel [this-uri super-types properties ranges]
  OntologyModel
  (type-uri [this]
    this-uri)

  (super-type-uris [this]
    (conj super-types this-uri))

  (add-property [this alias uri range]
    (let [prop-val (if (coll? uri) (apply rdf-resource uri) (rdf-resource uri))
  ...
  (remove-property-by-uri [this uri]
    (let [alias (property-alias this uri)]
      (if (nil? alias)
        this
  ...
  (remove-property-by-alias [this alias]
    (if (nil? alias)
      this
      (let [propsp (dissoc properties alias)
            rangesp (dissoc ranges alias)]
  ...
  (to-pattern [this props]
    (build-pattern-for-model this-uri ?s props properties))

  (to-map [this triples-or-vector]
    (let [triples (if (:triples (meta triples-or-vector))
                    triples-or-vector
  ...
  (property-uri [this alias]
    (alias properties))

  (property-range-description [this alias]
    (alias ranges))

  (property-alias [this uri]
    (first (filter #(= (str (get properties %1)) (str uri)) (keys properties))))

  (parse-prop-value [this alias val]
  ...
  (let [typeuri (if (coll? typeuri-pre) (apply rdf-resource typeuri-pre) (rdf-resource typeuri-pre))
        ...
        maps (reduce (fn [[ac-props ac-ranges] it]
                       (let [{uri :uri range :range} (it props-map-pre)
                             ...
                             range-val (if (supported-datatype? range)
                                         {:kind :datatype
                                          :range (rdf-resource (datatype-uri range))}
                                         {:kind :resource
                                          :range (if (coll? range)
                                                   (apply rdf-resource range)
                                                   range)})]
...
(defn extend-rdfs-schemas
  ...
  (let [props (flatten
        ...
        range-map (reduce (fn [acum prop]
                            (let [alias (if (nil? (get prop-map (:alias prop)))
                                          (:property prop)
                                          (:alias prop))]
...
(defn- parse-rdf-schema-from-model
  [model resource]
  (let [properties (flatten-1 (model-pattern-apply model [[?s rdfs:domain resource]
                                                          [?s rdfs:range ?o]]))
        prop-ranges (reduce (fn [acum [s p o]]
                              (if (= (str p) rdfs:range)
                                (let [prop-val (if (supported-datatype? (str o))
                                                 (datatype-symbol (str o))
                                                 (str o))]
